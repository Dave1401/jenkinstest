...
''') {
  node(POD_LABEL) {
    ...
    stage('Maven: Retrieve project') {
      git url: 'https://github.com/systemsexpert-pxl/simplewebservice-ci-pe2-Dave1401.git', branch: 'main'
      container('maven') {
    ...
    stage('kaniko: Build & Deploy Image') {
      container('kaniko') {
        stage('Build & Deploy to dockerhub') {
          env.TAG = myVersion
          sh '''
            /kaniko/executor --context `pwd` --destination daveke/simplewebservice:$TAG
            echo -e deployed daveke/simplewebservice:$TAG
          '''
        }
      }
    }
